...
% reading compressed video on Macintosh.  You can comment out the error
% and run this example with uncompressed stereo videos.
if strcmp(computer(), 'MACI64')
    error(['vision.VideoFileReader does not support code generation for ',... 
        'reading compressed video on Macintosh systems']);
end

% compressed file on Macintosh.
...
% Use the codegen function to compile the |visionRecovertformCodeGeneration_kernel|
...
visiondemo_dir = pwd;  % Store the current directory
mlcmexdir   = [tempdir 'DepthEstimationFromStereoVideo_mexdir']; % Name of
                                                    % temporary directory 
mkdir(mlcmexdir); % Create temporary directory
curdir = cd(mlcmexdir);
