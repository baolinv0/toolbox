...
% Y-coordinate of the pedestrian's feet.
    ...
    function obj = setupSystemObjects(videoFile,scaleDataFile)
        ...
        obj.videoPlayer = vision.VideoPlayer('Position', [2, 370, 1290, 750]);                
        
        % Create a people detector and initialize the parameters. The
        % MinSize and MaxSize parameters are set using a priori knowledge
        % that the height of a person is expected to be between [96, 384]
        % pixels. ScaleFactor and WindowStride may be lowered to increase
        % accuracy at the cost of processing speed.
...
%                         classification score at the corresponding frame.
    ...
    function [centroids, bboxes, scores] = detectPeople()
        ...
        [bboxes, scores] = step(obj.peopleDetector, rgb2gray(frame), option.ROI);
