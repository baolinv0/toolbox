 ...
 */
  ...
  #define VIP_SNPRINTF(out, size, in, data,result)   \
     _set_output_format(_TWO_DIGIT_EXPONENT); /* g746689 */ \
...
LIBMWVISIONRT_API void MWVIP_snprintf(char_T* outbuf, 
				...
				int_T itemDataType,boolean_T isString, int_T maxChars)
{
	...
	if(isString)
	{
        ...
        VIP_SNPRINTF(outbuf, maxChars, formatString, stringIn, charsPrinted);
        return;
	}
...
LIBMWVISIONRT_API void MWVIP_snprintf_wrapper(void* outbuf, 
